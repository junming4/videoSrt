...
class food:
    ...
    def downAndGetSrt(self):
        ...
        for urlText in urlList:
            ...
            try:
                ...
                p = system("autosub %s -o %s" % (file_name, srtName))
    ...
    def merge(self):
        ...
        for urlText in urlList:
            ...
            target = '%s/%s.mp4' % (target_path, name)


            if not os.path.isfile(file_name) or not os.path.isfile(srtName) or os.path.isfile(target):
                continue

            try:

                system('ffmpeg -i %s -vf subtitles=%s  %s'%(file_name,srtName, target))
